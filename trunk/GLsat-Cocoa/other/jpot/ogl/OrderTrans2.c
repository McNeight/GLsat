...
void ReadData(void) {
	if ((fp = fopen(datafile, "r")) == NULL)
	{
		...
		while ( !feof(fp)) {
		  fscanf(fp," %f %f %f %f %f %f %f %d", &JPOT$1, &JPOT$2, &JPOT$3, &JPOT$4, &JPOT$5, &JPOT$6, &JPOT$7, &JPOT$swap);
		}
		fclose(fp);
	}
}
...
{	
  ...
  glutCreateWindow("The Order of Transformations");
  init(); 
