...
char VersionStr[] = "N3EMO Orbit Simulator  v2.2";
...
GetSatelliteParams()
{
    ...
    sscanf(str,"%ld",&EpochOrbitNum);
        ...
        while (1)
            ...
            if (strlen(str) <= 2)
                break;  /* Blank line */
            if (sscanf(str,"Beacon: %lf",&BeaconFreq) == 1)
                break;  /* found it */
            }
    ...
    if (found)
        {
        MatchStr(InFile,"mode.dat","Perigee phase:");
        fgets(str,100,InFile);
        sscanf(str,"%lf",&perigeePhase);

        MatchStr(InFile,"mode.dat","Max phase:");
        fgets(str,100,InFile);
        sscanf(str,"%lf",&MaxPhase);
...
main()
{
    ...
    for (CurrentTime = StartTime-StepTime; CurrentTime <= EndTime;
        ...
        /* Fix these later to correct for drag  */
